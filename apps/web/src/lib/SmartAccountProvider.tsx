...
export function SmartAccountProvider({ children }: { children: React.ReactNode }) {
  ...
  const { initCustomAccount, isLoading: isCustomLoading, error: customError, isReady } =
    CustomSmartAccount();
  ...
  const initialize = useCallback(
    async (force = false): Promise<SmartAccountClient | null> => {
      ...
      const promise = (async (): Promise<SmartAccountClient | null> => {
        ...
        // Pre-checks
        if (!isConnected) {
          console.debug("[SmartAccount] init skipped: wallet not connected");
          return null;
        }
        if (!address) {
          console.debug("[SmartAccount] init skipped: no address found");
          return null;
        }

        // CRITICAL: Check if wallet client is ready
        if (!isReady) {
          console.debug("[SmartAccount] init skipped: wallet client not ready yet");
          return null;
        }

        console.debug("[SmartAccount] Starting initialization for address:", address);
  ...
  // attempt init when wallet connected AND wallet client is ready
  useEffect(() => {
    if (isConnected && address && isReady) {
      ...
      // reset state when user disconnects or wallet changes
      setClient(null);
      setError(null);
      clearRetry();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isConnected, address, isReady]); // Added isReady dependency

  // re-trigger initialization on window focus if not initialized
  useEffect(() => {
    const onFocus = () => {
      if (!client && isConnected && address && isReady) {
        initialize().catch(() => {});
      }
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [client, isConnected, address, isReady]); // Added isReady dependency
